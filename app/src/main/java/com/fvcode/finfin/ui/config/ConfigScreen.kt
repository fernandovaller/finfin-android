...
private fun AbaGeral(estado: ConfigUiState, tema: String, aoTema: (String) -> Unit) {
    ...
    Text("Servidor: ${BuildConfig.BASE_URL}", style = MaterialTheme.typography.bodySmall)
}
...
private fun AbaBackup(
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        Button(enabled = !ocupado && demo?.existe != true, onClick = aoGerarDemo) { Text("Gerar demo") }
        OutlinedButtonBotao(enabled = !ocupado && demo?.existe == true, onClick = aoRemoverDemo, texto = "Remover demo")
    }

    SecaoTitulo("Exportar")
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        Button(enabled = !ocupado, onClick = aoExportarJson, modifier = Modifier.weight(1f)) {
            Text("Backup JSON")
        }
    }
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        Button(enabled = !ocupado, onClick = { aoExportarCsv("receitas") }, modifier = Modifier.weight(1f)) {
            Text("CSV receitas")
        }
        Button(enabled = !ocupado, onClick = { aoExportarCsv("despesas") }, modifier = Modifier.weight(1f)) {
            Text("CSV despesas")
        }
    }

    SecaoTitulo("Importar / restaurar")
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        Button(enabled = !ocupado, onClick = aoImportar, modifier = Modifier.weight(1f)) {
            Text("Importar backup")
        }
        OutlinedButtonBotao(enabled = !ocupado, onClick = aoRestaurar, texto = "Repor padrões")
    }
    if (ocupado) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center) {
            CircularProgressIndicator()
...
@Composable
private fun OutlinedButtonBotao(enabled: Boolean, onClick: () -> Unit, texto: String) {
    androidx.compose.material3.OutlinedButton(enabled = enabled, onClick = onClick) { Text(texto) }
}
...
private fun AbaEmail(estado: ConfigUiState, ocupado: Boolean, aoSalvar: (String) -> Unit) {
    ...
    Button(enabled = !ocupado, onClick = { aoSalvar(chave); chave = "" }) { Text("Salvar chave") }
}

@Composable
private fun AbaPerigo(ocupado: Boolean, aoApagarLancamentos: () -> Unit, aoApagarTudo: () -> Unit) {
    SecaoTitulo("Zona de perigo")
    Text(
